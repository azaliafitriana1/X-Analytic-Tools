...
# B. Stopwords Khusus Topik (UPDATE FINAL BERDASARKAN HASILMU)
# UPDATE LIST INI DI app.py
TOPIC_STOPWORDS = {
    ...
    # --- KATA PENTING YANG SAYA "SELAMATKAN" (DIHAPUS DARI STOPWORD) ---
    # 'cancel', 'mahal', 'murah', 'biaya', 'harga', 'lama', 'hujan', 'macet', 'nunggu', 'cepet'
    # Kata-kata di atas sekarang BOLEH muncul di topik.
...
# DEFAULT_FILE = "data/gojek.csv"
# TOMBOL PEMBERSIH CACHE
...
#     df = load_dataset(DEFAULT_FILE)
    ...
    st.sidebar.warning("Silakan upload dataset terlebih dahulu.")

# ... (di bawah file uploader) ...
if df is not None:
    # Konversi DF ke CSV string
...
# --- 1. BERANDA (EXECUTIVE DASHBOARD) ---
    if menu == "🏠 Beranda":
        ...
        with col_right:
            ...
            try:
                vec = CountVectorizer(max_features=10, stop_words=list(FINAL_STOP_SET))
    ...
    # --- 2. SENTIMEN ---
        ...
        with c2:
            pil = st.selectbox("Filter:", ["Semua", "Negatif", "Positif", "Netral"])
            view = df_organic if pil == "Semua" else df_organic[df_organic['sentiment'] == pil]
            st.dataframe(
                view[['created_at', 'username', 'full_text', 'sentiment']],
                column_config={"full_text": st.column_config.TextColumn("Tweet", width="large")},
